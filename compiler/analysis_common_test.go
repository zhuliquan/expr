	...
	for _, tt := range []testCase{
		...
		},
		{
			...
			program: &vm.Program{
				...
				Arguments: []int{0, 21, 0, 0, 4, 0, 0, 21, 0, 0},
			},
		},
		{
			input: `true ? 1 : 2`,
		...
		},
		{
			...
			expr:  `{"1": true, "2": true, "3": false, "4": 1.7}`,
			program: &vm.Program{
				Constants: []interface{}{1.7, 4},
				Bytecode: []vm.Opcode{
					vm.OpPush,
		...
		t.Run(tt.input, func(t *testing.T) {
			...
			if err != nil {
				...
				assert.Equal(t, tt.expr, compiler.Identifier(program.Node()))
